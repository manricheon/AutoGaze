...
시각화 버전(단계별 시뮬레이터 포함)은 `v06-pipeline.html`에 있다 — 클립 3종(모션 상위 / 장면전환 /
거의 정지) × 예산 비율 3종(25 · 50 · 75%) × 16 튜블렛 전체를 단계별로 넘겨볼 수 있고, 각 손질
단계에는 **직전 대비 변화량(diff) 지도**가 함께 붙는다. 실제 코드가 계산한 값만 임베드했고
이미지가 base64로 들어가 용량이 크므로 **추적하지 않는다**(`.gitignore`). 생성 방법은 §7.
...
flowchart TB
  ...
  subgraph P1["② 세 층의 신호"]
    MD["움직임 (모션)<br/>프레임 차이"]:::mot
    GR["윤곽 (엣지)<br/>밝기 기울기"]:::app
    DG["영역 (region)<br/>DoG로 계산"]:::app
  end
  ...
  subgraph P3["④ 하나의 점수로"]
    AW["섞는 비율 자동 결정<br/>움직임 적으면 → 윤곽·영역 쪽"]:::io
    FB["세 층 합치기<br/>넓게 퍼진 건 깎고"]:::io
...
색: 파랑 = 모션/시간, 오커 = 윤곽·영역(모양), 자주 = v0.6 신규, 청록 = 곱해서 깎는 문.
점선 = 곱셈·예산 경로.
...
| 03 | 윤곽(엣지) 찾기 | `spatial_op="grad"` | 밝기가 급변하는 곳. `√(dx²+dy²)`, 튜블렛 평균 위에서 |
...
| 07 | 섞는 비율 자동 결정 | `motion_weight="auto"` | `w = E[모션]/(E[모션]+E[윤곽])`. 32f 실측 0.34, 정지 클립 0.03 → **움직임이 적을수록 윤곽·영역(물체·글자) 쪽으로 기운다** |
| 08 | 세 층 합치기 | `fusion_norm="peak"` + `dog_blob_weight=0.5` | 화면 전체에 고르게 퍼진 지도는 **가중치가 깎인다**(뾰족한 지도 우대) → 지도 수준 무늬 억제 + 카메라 흔들림 자동 무시. 여기서 세 번째 층 **영역(region)**(DoG로 계산)이 합류한다 — **모션 / 윤곽 / 영역** 3층 구조이고, 모션과 윤곽은 선에서만 발화하므로 균일한 색의 몸통은 둘 다 0이다. 캡션은 객체 단위이니 실루엣만이 아니라 **몸통 패치**가 있어야 인식된다 |
...
| 11 | 멈춘 곳 살려주기 **v0.6** | `static_guard` | **국면 전환**이 핵심. 07의 비율은 클립 전역 값 하나인데, 이건 그 순간이 멈춰 있을 때만 국소적으로 윤곽 힘을 되돌린다. 움직임 큰 순간은 영향 0. 글자·문서·고정 샷이 살아남는 이유. **v0.6 네 손질 중 로컬 지표에서 유일하게 이긴 것**(+0.012) |
...
점수 지도가 만들어진 다음이 진짜 선택이다. 아래는 `videos/internvid_eval16/0TjQiQFeum0_t0.1-4.0_fps15.9.mp4`
(모션 상위 클립), **32프레임** 384px에서 나온 **실제 값**이다. 32프레임은 2프레임씩 묶여 16개
튜블렛이 되고, 선택은 튜블렛 단위로 일어난다.

```
격자   16 순간 × 24 × 24 = 9,216칸          (한 순간당 576칸)
```

| 예산 비율 | 총예산 | 바닥 m | 상한 k_gate | 경쟁 몫 | 균등 몫 | 순간별 결과(16개) |
|---|---|---|---|---|---|---|
| 0.25 | 2,304 | 36 | 288 | 1,728 | 144 | 84 104 132 136 100 140 184 224 144 116 152 108 96 248 164 172 |
| 0.5 | 4,608 | 72 | 576 | 3,456 | 288 | 264 288 316 296 284 300 352 324 284 240 288 196 236 328 300 312 |
| 0.75 | 6,912 | 108 | 576 | 5,184 | 432 | 380 424 468 424 404 432 476 500 428 424 420 420 424 440 416 432 |

세 비율을 나란히 보면 두 가지가 드러난다.

- **예산이 커질수록 배분이 균등에 수렴한다.** 0.25에서는 84~248칸(균등 144 대비 ±70%)으로 크게
  벌어지지만, 0.75에서는 380~500칸(균등 432 대비 ±15%)으로 좁아진다. 예산이 넉넉해지면 좋은
  순간이 먼저 포화되고 남는 예산이 결국 모든 순간으로 흘러간다 — **내용 적응성은 예산이 빡빡할 때
  가장 크게 작동한다.**
- **상한(k_gate)은 높은 비율에서 무력해진다.** 0.5 이상에서 `k_gate = 576 = 한 순간의 전체 칸 수`로
  포화되므로 제약이 사라진다. 독점 방지 장치는 설계상 낮은 비율에서만 의미가 있다.
...
1. **바닥 확보** — 순간마다 점수 상위 `m`칸(비율 0.25에서 36칸)을 무조건 확정.
2. **남은 예산 경쟁** — 남은 `K_total − T·m`칸(1,728칸)을 확정분 제외 전체에서 한 번에 뽑는다.
3. **독점 방지** — 한 순간이 노출할 후보를 균등 몫의 2배(비율 0.25에서 288칸)로 제한. 안 하면 문이 거의 다 열려
   덩어리 효과가 사라진다(경험적 발견).
...
자동으로 더 많은 칸을 가져간다 — 위 결과가 균등 몫 144칸에서 84~248칸으로 벌어진 이유다.
**균등** 방식에서는 점수만 올려도 칸 수가 고정이라 아무 일도 없으므로, 장면 기준 프레임 우대는
할당 단계에서 직접 칸을 옮기는 별도 경로를 갖고 있다.

> **예산은 정확히 맞는다.** 위 세 비율 모두 순간별 합이 총예산과 정확히 일치한다(2,304 / 4,608 / 6,912). 클립 3개 × 비율 3개 = 9조합 전부 확인했다.
