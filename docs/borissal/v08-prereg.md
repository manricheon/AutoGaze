...
### 2026-07-29 A7 — 양-ratio 승자 요건 추가 (A6 일부 대체, Stage B-0.5 확장)

사용자 지시(2026-07-29): 최종 채택 구성은 0.25와 0.5 **모두에서** 이겨야 한다.
A6은 dev-60 @0.5 판정을 생략했으나, 이는 "후보 간 0.5 비교"라는 결정을
Stage C(승자+기본+random만 포함)가 커버할 수 없다는 점을 놓쳤다 — 정정한다.

사전 확인(기록): 0.5 예산에서도 fine 변형들은 앵커 포화로 붕괴하지 않고
서로 다른 선택을 유지함을 셀렉터 마스크 대조로 확인(6클립, λ0.25/λ0.75/
wst0.5/기본 모두 상이). 0.5 비교는 유의미하다.

**Stage B-0.5 확장 (dev-60, ratio 0.5):**
- 로스터: 0.25 상위 2 (fine+λ0.75, fine+λ0.25) + v0.7 기본 + random.
  (wst0.5는 0.25에서 최하위·유의미 열세라 제외 — 비용 절약.)
- 프로토콜 동일: KV 캐시 생성기, 프레임 근거 심판, 순서 스왑 2회, 불일치=tie.
- 쌍: 각 후보 vs-random@0.5 (3) + 각 구성 vs-dense (4) = 클립당 7쌍.

**최종 라운드 승자 규칙 (양-ratio; 기존 0.25 단독 규칙을 대체):**
1. 게이트: 양 ratio 모두에서 vs-random 부호검정 p<0.05 승리. 실패 구성 탈락.
2. 게이트 통과 구성 중 min(승률@0.25, 승률@0.5) 최대.
3. 동률(±2%p) 시 양 ratio 평균 vs-dense 패배율 최소.

정직성 기록: 이 규칙은 0.25 결과(λ0.25 63.3% > λ0.75 61.7% > 기본 60.0%)를
이미 안 상태에서 등록된다. 그러나 0.5 데이터는 캡션 1장도 존재하지 않는
시점이며, 규칙의 형태(min-max + 동일 동률 창 + 동일 타이브레이크)는 기존
0.25 규칙의 대칭 확장이다. 0.5 결과를 보고 규칙을 바꾸는 일은 없다.

**Stage C에의 영향:** Stage C 심판은 Stage B-0.5 완료 후 확정 승자로 진행.
현재 생성 중인 Stage C 캡션(λ0.75 arm)은 승자가 바뀌면 λ0.25 arm을 추가
생성(생성은 의사결정 오염이 아님; holdout "1회" 원칙은 심판·집계에 적용).

### 2026-07-30 A8 — Stage C 로스터·실행 방식 확정 (holdout 심판 개시 전 기록)

A7 규칙의 승자는 v0.7 기본(타이브레이크: 양 ratio 평균 vs-dense 패배율 최소).
Stage C 로스터를 다음으로 확정한다:
- **v0.7 기본** (확정 승자) — 성공 기준 판정 대상
- **fine+λ0.75** (0.25 특화 관찰 arm) — 0.25 단독 규칙의 승자였으므로
  ratio-조건부 프리셋 가설의 증거 수집용. 이 arm의 결과로 이번 라운드
  프리셋 결정을 하지 않는다(관찰 전용, winner's curse 가드 유지).
- **random** (하한 대조)
쌍: 각 selector vs-random@동일비율, 각 구성 vs-dense (random 포함) =
클립·ratio당 5쌍, 양 ratio, 스왑 2회, 3회 반복 다수결 = 총 7,200 판정.
실행: 3회 반복은 **독립 패스 3회를 순차 세션으로 분할** 실행한다(세션당
서브에이전트 상한 때문). 반복 1이 끝나기 전에는 어떤 집계도 산출하지
않으며, 3패스 완료 후 단 1회 집계한다 — holdout "1회 원칙"은 심판
호출이 아니라 **집계·의사결정 1회**에 적용됨을 명확히 한다.
