...
`autogaze_latency_ms` is inclusive: it records preprocessing needed to build the AutoGaze inputs plus the AutoGaze selector/result stage over all processed frames or crops. This applies to `sample`, `chunk`, `interval`, and `all`. For `resize_then_chop`, it includes resize/chop expansion time and the work over all processed crop frames. For interval runs, `frame_interval`, `number_of_source_frames`, `number_of_processed_frames`, `autogaze_latency_source_frame_count`, and `autogaze_latency_processed_frame_count` show exactly what the timing covers. The split fields `autogaze_preprocessing_latency_ms`, `autogaze_stage_latency_ms`, `autogaze_model_forward_latency_ms`, and `autogaze_result_build_latency_ms` are also reported, along with normalized `autogaze_latency_per_source_frame_ms` and `autogaze_latency_per_processed_frame_ms`. `visualization_latency_ms` and `wall_clock_latency_ms` are recorded separately. Use `--no-progress` to disable command-line progress bars while keeping the same timing behavior.
