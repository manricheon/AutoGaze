...
- `result.processing_budget_summary`: 원본 해상도, runner-side resize 후 processor 입력 해상도, 프레임 수, 프레임당 spatial tile 수, thumbnail 적용 여부, multi-scale patch breakdown, AutoGaze 전후 encoder patch budget과 LLM visual-token budget을 한 번에 읽기 위한 요약입니다. `stream-profile`에도 같은 이름으로 저장됩니다.
...
HLVid benchmark의 per-row `*_predictions.jsonl`뿐 아니라 `*_summary.json`과 `hlvid_autogaze_gain_report.json`에도 `processing_budget_summary`가 들어갑니다. Summary/gain report에서는 숫자 필드는 median, 해상도나 scale breakdown 같은 non-numeric 필드는 첫 non-null 값을 대표값으로 둡니다. 그래서 나중에 Markdown으로 변환할 때도 `## Processing Budget Summary`에서 resize/tiling/thumbnail/multiscale patch budget을 바로 볼 수 있습니다.

raw output JSON이 너무 길면 `--print-summary --summary-json <path>`를 붙이세요. 전체 raw JSON은 `--output-json`에 그대로 저장하고, 터미널과 summary file에는 답변과 함께 `prompt`, `question`, `video_input_summary`, `processing_budget_summary`, `autogaze_token_summary`, `key_autogaze_effect`, `latency_accounting`를 별도로 정리합니다. `single` 모드에서는 `prompt`가 실행에 사용한 `--prompt` 원문이고, HLVid row 기반 실행에서는 per-row `question`이 `predictions.jsonl`과 `scored_predictions.jsonl`에 보존됩니다. `video_input_summary`에는 원본 총 프레임 수, 원본 해상도, 요청한 video/thumbnail frame 수, 실제 processor tensor 기준 frame 수, runner resize 적용 여부, resize 후 processor 입력 해상도, decode 전략/읽은 frame 수, spatial tile/temporal chunk 수가 들어갑니다. `processing_budget_summary`는 이 입력 정보를 patch/token budget과 묶어서 보여줍니다. 특히 `model_processing_unit`, `tiling.spatial_tiles_per_frame`, `thumbnail`, `multiscale_patch_space`, `patch_budget_before_siglip`, `llm_visual_budget`를 먼저 보면 실험마다 “전체 프레임/타일/멀티스케일 patch 대비 AutoGaze가 얼마나 줄였는지”를 빠르게 볼 수 있습니다. `autogaze_token_summary`에는 사용한 프레임/타일 기준 raw patch budget과 AutoGaze가 실제 유지한 patch 수, TokenShuffle 이후 LLM visual token 수가 나뉘어 들어갑니다. `key_autogaze_effect`에는 AutoGaze 전후 차이를 가장 잘 보여주는 encoder patch 수, LLM visual token 수, reduction ratio/percent, SigLIP/MLLM 계산량 감소 추정치, 핵심 latency/memory median이 모입니다. 상세 분석용 `latency_ms`, `memory_bytes`, `tokens`, `compute` 섹션도 함께 남깁니다.
...
| 영역 | 필드 | 설명 | 코드 위치 | 중요도 |
| --- | --- | --- | --- | --- |
| 입력/샘플링 | `video_input_summary.*`, `processing_budget_summary.video.*`, `input_token_count`, `input_shapes`, `token_metrics.video_sampled_frames`, `token_metrics.thumbnail_sampled_frames` | 원본 프레임 수/해상도, 요청 frame 수, 실제 processor tensor 기준 frame 수, resize 후 입력 해상도, text+visual context 길이입니다. 설정이 의도대로 반영됐는지 확인합니다. | [build_video_input_summary](../repro/nvila_runner.py), [build_processing_budget_summary](../repro/nvila_runner.py), [generate_one](../repro/nvila_runner.py), [compute_visual_token_metrics](../repro/nvila_runner.py), [build_stream_profile_token_metrics](../repro/nvila_runner.py) | 높음 |
| 타일/멀티스케일 patch budget | `processing_budget_summary.model_processing_unit`, `processing_budget_summary.tiling`, `processing_budget_summary.thumbnail`, `processing_budget_summary.multiscale_patch_space`, `processing_budget_summary.patch_budget_before_siglip`, `processing_budget_summary.llm_visual_budget` | 프레임당 타일 수, 392px tile 처리 단위, thumbnail keep-all 여부, scale별 patch 수, AutoGaze 전 full patch budget, AutoGaze 후 selected patch budget, TokenShuffle 이후 LLM visual token 감소량입니다. AutoGaze 적용 이득을 가장 직관적으로 보는 요약입니다. | [build_processing_budget_summary](../repro/nvila_runner.py), [build_mllm_processing_budget_summary](../repro/plugins/mllm_adapters.py) | 높음 |
...
- `generation.metrics.processing_budget_summary`: Qwen/flexible runner의 resize request, thumbnail append 여부, Qwen `video_grid_thw` 처리 단위, spatial chunk limit, estimated/actual visual token budget을 모아 둔 요약입니다. 실제 processor가 `video_grid_thw`를 만든 실행에서는 `patch_budget_before_vit.actual_video_grid_thw`와 `actual_raw_patch_tokens_before_vit`도 기록됩니다.
