...
def help_commands(python_executable: str) -> list[tuple[str, list[str]]]:
    return [
        ...
        ("external_cuda_notebook_help", [python_executable, "scripts/write_external_cuda_verification_notebook.py", "--help"]),
...
def dry_run_commands(python_executable: str, temp_root: Path) -> list[tuple[str, list[str]]]:
    ...
    return [
        (
            "hlvid_nvila_prepare_only",
            [
                python_executable,
                "scripts/run_hlvid_folder_benchmark.py",
                "--dataset-dir",
                str(mini_hlvid),
                "--manifest",
                str(mini_manifest),
                ...
                str(mini_videos),
                "--output-dir",
                str(temp_root / "nvila_hlvid_prepare_only"),
                "--prepare-only",
                "--limit",
                "1",
            ],
        ...
        (
            "hlvid_qwen_plugin_route_limit0",
            [
                python_executable,
                "scripts/run_hlvid_folder_benchmark.py",
                "--manifest",
                ...
                str(mini_videos),
                "--output-dir",
                str(temp_root / "qwen_plugin_route_limit0"),
                "--plugin-suite",
                "qwen",
                "--limit",
                "0",
        ...
        (
            "flexible_qwen_sparse_single_dry_run",
            [
                python_executable,
                "-m",
                ...
                "16",
                "--output-json",
                str(temp_root / "flexible_qwen_sparse_single_dry_run.json"),
            ],
        ),
        (
...
def focused_tests() -> list[str]:
    return [
        ...
        "tests/test_external_cuda_verification_notebook.py",
...
def check_plugin_qwen_routing() -> dict[str, Any]:
    ...
    common = {
        ...
        "autogaze_target_scales": "64+128+192+224",
...
def entrypoint_matrix() -> list[dict[str, str]]:
    return [
        ...
        },
        {
            ...
            "vit": "NVILA-HD SigLIP",
            "mllm": "NVILA-HD",
            "verification": "help + prepare-only layout check + wrapper tests; actual CUDA HLVid required separately",
        },
        {
            ...
            "mllm": "Qwen VL",
            "verification": "help + limit-0 wrapper route + route forwarding checks + plugin tests",
        },
        {
            ...
            "mllm": "Qwen VL",
            "verification": "inspect checks + single dry-run plan; actual model generate required separately",
        },
        {
