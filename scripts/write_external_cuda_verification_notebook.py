    ...
    *,
    ...
    cells = [
        ...
        code_cell(
            ...
            "        '--visualization-output-dir', OUTPUT_ROOT / 'visualizations',\n"
        ...
        code_cell(
            ...
            "        '--visualization-output-dir', OUTPUT_ROOT / 'nvila_single_visualizations',\n"
            "        '--visualization-fps', '4',\n"
            "        '--max-new-tokens', '1',\n"
            "    ]\n"
        ...
        code_cell(
            "if RUN_QWEN_SINGLE:\n"
            "    qwen_single_out = OUTPUT_ROOT / 'qwen_single_smoke'\n"
            "    qwen_single_out.mkdir(parents=True, exist_ok=True)\n"
            "    qwen_common = [\n"
            "        sys.executable, '-m', 'repro.flexible_runner',\n"
            "        '--mode', 'single',\n"
            "        '--model-family', 'qwen3-vl',\n"
            "        '--model-path', WEIGHTS_ROOT / 'Qwen__Qwen2.5-VL-3B-Instruct',\n"
            "        '--vision-encoder-adapter', 'qwen3-vl-vision',\n"
            "        '--mllm-adapter', 'qwen3-vl',\n"
            "        '--video', 'inputs/hlvid_example/clip_av_video_5_001.mp4',\n"
            "        '--prompt', 'Describe the video in one short sentence.',\n"
            "        '--device-map', 'auto', '--dtype', 'float16', '--attn-implementation', 'eager',\n"
            "        '--num-video-frames', '16', '--qwen-video-nframes', '16',\n"
            "        '--num-video-frames-thumbnail', '0', '--qwen-thumbnail-mode', 'none',\n"
            "        '--max-tiles-video', '1', '--qwen-vit-chunk-frames', '16', '--qwen-vit-max-spatial-chunks', '1',\n"
            "        '--video-resize-longest-edge', '224', '--video-decode-strategy', 'seek',\n"
            "        '--max-new-tokens', '1',\n"
            "    ]\n"
            "    run([*qwen_common, '--token-selector-adapter', 'keep-all', '--autogaze-integration-level', 'none', '--qwen-vit-mode', 'qwen_full_vit', '--output-json', qwen_single_out / 'qwen_full_vit.json'], cwd=REPO_DIR)\n"
            "    run([*qwen_common, '--token-selector-adapter', 'keep-all', '--autogaze-integration-level', 'none', '--qwen-vit-mode', 'qwen_chunked_vit', '--output-json', qwen_single_out / 'qwen_chunked_vit.json'], cwd=REPO_DIR)\n"
            "    run([\n"
            "        *qwen_common,\n"
            "        '--token-selector-adapter', 'autogaze', '--autogaze-integration-level', 'pre_encoder_sparse', '--pre-encoder-prune-adapter', 'autogaze-sparse',\n"
            "        '--qwen-vit-mode', 'qwen_chunked_vit_autogaze_sparse', '--run-autogaze-selector',\n"
            "        '--autogaze-model', WEIGHTS_ROOT / 'nvidia__AutoGaze', '--autogaze-device', 'cuda', '--autogaze-dtype', 'float16',\n"
            "        '--autogaze-target-scales', '64+128+192+224', '--autogaze-target-patch-size', '16', '--autogaze-tile-size', '224',\n"
            "        '--autogaze-chunk-frames', '16', '--max-batch-size-autogaze', '2',\n"
            "        '--gazing-ratio', '0.1',\n"
            "        '--output-json', qwen_single_out / 'qwen_chunked_vit_autogaze_sparse.json',\n"
            "    ], cwd=REPO_DIR)\n"
            "    print('qwen_single_outputs:', qwen_single_out)\n"
        ),
        code_cell(
        ...
        code_cell(
            ...
            "            'answer': 'B',\n"
            "        }) + '\\n', encoding='utf-8')\n"
            "    vjepa_hlvid_out = OUTPUT_ROOT / 'vjepa_qwen_hlvid_mini'\n"
            "    vjepa_hlvid_out.mkdir(parents=True, exist_ok=True)\n"
            "    run([\n"
            "        sys.executable, '-m', 'repro.vjepa_qwen_hlvid_benchmark',\n"
            "        '--manifest', manifest,\n"
            "        '--video-root', REPO_DIR / 'inputs/hlvid_example',\n"
            "        '--output-dir', vjepa_hlvid_out,\n"
            "        '--vjepa-qwen-modes', 'dense_off,autogaze_single_grid',\n"
            "        '--vjepa-model', WEIGHTS_ROOT / 'facebook__vjepa2-vitl-fpc64-256',\n"
            "        '--qwen-model', WEIGHTS_ROOT / 'Qwen__Qwen2.5-VL-3B-Instruct',\n"
            "        '--autogaze-model', WEIGHTS_ROOT / 'nvidia__AutoGaze',\n"
            "        '--device', 'cuda', '--dtype', 'float16', '--require-cuda',\n"
            "        '--num-video-frames', '16', '--frames-per-clip', '16',\n"
            "        '--video-resize-longest-edge', '224', '--video-decode-strategy', 'seek',\n"
            "        '--visualization-max-frames', '16',\n"
            "        '--max-batch-size-autogaze', '2', '--max-new-tokens', '1',\n"
            "        '--gazing-ratio', '0.1',\n"
            "        '--limit', '1', '--continue-on-error',\n"
            "    ], cwd=REPO_DIR)\n"
            "    vjepa_summary = vjepa_hlvid_out / 'vjepa_qwen_hlvid_summary.json'\n"
            "    assert vjepa_summary.exists(), vjepa_summary\n"
            "    print('vjepa_qwen_hlvid_mini_outputs:', vjepa_hlvid_out)\n"
        ),
    ]
    return {
        "cells": cells,
